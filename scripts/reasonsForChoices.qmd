...
### 🟧co-occurrece analysis

```{r}
# data_reasons_all

co_matrix <- t(as.matrix(data_reasons_all)) %*% as.matrix(data_reasons_all)

co_matrix
```

```{r}
co_prop <- co_matrix / nrow(data_reasons_all)
```
...
```{r}
tetra <- tetrachoric(data_reasons_all)$rho
```

```{r}
g <- graph_from_adjacency_matrix(tetra, weighted = TRUE, diag = FALSE)
