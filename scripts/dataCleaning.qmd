...
reason <- "Info Sharing diagnostics (Cook's)"

filtered_data <- filtered_data %>%
  mutate(
    Why_Exclude = if_else(ResponseId %in% ids, reason, Why_Exclude),
    Include     = if_else(ResponseId %in% ids, 0L, Include)  # flip Include to 0
  )
